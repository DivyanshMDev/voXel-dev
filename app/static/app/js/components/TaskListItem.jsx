...
class TaskListItem extends React.Component {
  ...
  shouldRefresh() {
    ...
    if (
      [statusCodes.COMPLETED, statusCodes.FAILED, statusCodes.CANCELED].indexOf(
  ...
  unloadTimer() {
    ...
    if (this.state.task.processing_time)
  ...
  componentDidMount() {
    if (this.shouldRefresh())
      this.refreshTimeout = setTimeout(
        () => this.refresh(),
        this.props.refreshInterval || 3000
      );
  ...
  refresh() {
    // Fetch
    this.refreshRequest = $.getJSON(
      `/api/projects/${this.state.task.project}/tasks/${this.state.task.id}/`,
      (json) => {
        if (json.id) {
      ...
      if (result.status === 404 || errorThrown === "Not Found") {
      ...
      const doAction = () => {
        ...
        $.post(url, {
          ...
          .fail(() => {
            this.setState({
              actionError:
                options.defaultError || _("Cannot complete operation."),
              actionButtonsDisabled: false,
            });
          })
          .always(() => {
  ...
  checkForCommonErrors(lines) {
    for (let line of lines) {
      if (
        ...
        line.indexOf("SVD did not converge") !== -1 ||
        line.indexOf("0 partial reconstructions in total") !== -1
      ) {
        this.setState({
          friendlyTaskError: interpolate(
            _(
              "It looks like there might be one of the following problems: %(problems)s You can read more about best practices for capturing good images %(link)s."
            ),
            {
              problems: `<ul>
          ...
          <li>${_(
            "The min-num-features task option is set too low, try increasing it by 25%"
              ...
              link: `<a href='https://docs.webodm.net/references/create-successful-maps' target='_blank'>${_(
                "here"
              )}</a>`,
            }
          ),
        ...
        line.indexOf("Illegal instruction") !== -1 ||
        line.indexOf("Child returned 132") !== -1
      ) {
        this.setState({
          friendlyTaskError: interpolate(
            _(
              "It looks like this computer might be too old. WebODM requires a computer with a 64-bit CPU supporting MMX, SSE, SSE2, SSE3 and SSSE3 instruction set support or higher. You can still run WebODM if you compile your own docker images. See %(link)s for more information."
            ),
            {
              link: `<a href='https://github.com/OpenDroneMap/WebODM#common-troubleshooting'>${_(
                "this page"
              )}</a>`,
            }
          ),
        ...
        this.setState({
          friendlyTaskError: _(
            "The processing node is missing a program necessary to complete the task. This might indicate a corrupted installation. If you built OpenDroneMap, please check that all programs built without errors."
          ),
        });
      }
    }
  ...
  getRestartSubmenuItems() {
    ...
    for (let rfParam in rfMap) {
      rfMap[rfParam].label = interpolate(_("From %(stage)s"), {
        stage: rfMap[rfParam].label,
      });
    ...
    if (
      items.length > 0 &&
      [statusCodes.CANCELED, statusCodes.FAILED].indexOf(task.status) !== -1
    ) {
      ...
      items.unshift({
        ...
        onClick: this.genRestartAction(
          task.can_rerun_from[task.can_rerun_from.length - 1]
        ),
      });
    }
  ...
  genRestartAction(rerunFrom = null, options = {}) {
    ...
    const setTaskRerunFrom = (value) => {
      ...
      return $.ajax({
        ...
        .fail(() => {
          this.setState({
            actionError: interpolate(_("Cannot restart task from (stage)s."), {
              stage: value || "the start",
            }),
            actionButtonsDisabled: false,
          });
        });
  ...
  render() {
    ...
    const name =
      task.name !== null
        ? task.name
        : interpolate(_("Task #%(number)s"), { number: task.id });
    ...
    if (
      ...
      status = _("Waiting for a node...");
    if (task.pending_action !== null)
      status = pendingActions.description(task.pending_action);
    ...
    if (this.state.expanded) {
      let showOrthophotoMissingWarning = false,
        showMemoryErrorWarning =
          this.state.memoryError && task.status == statusCodes.FAILED,
        showTaskWarning =
          this.state.friendlyTaskError !== "" &&
          task.status == statusCodes.FAILED,
        showExitedWithCodeOneHints =
          task.last_error === "Process exited with code 1" &&
          !showMemoryErrorWarning &&
          !showTaskWarning &&
          task.status == statusCodes.FAILED,
        memoryErrorLink = this.isMacOS()
          ? "http://stackoverflow.com/a/39720010"
      ...
      if (
        ...
        addActionButton(
          ...
          this.genActionApiCall("cancel", {
            defaultError: _("Cannot cancel task."),
          })
        );
      }
      ...
      if (
        ...
        addActionButton(
          ...
          this.genRestartAction(rerunFrom, {
            confirm: _("Are you sure you want to restart this task?"),
          }),
          {
                  ...
                  <ul key="dropdown-menu" className="dropdown-menu">
                    {subItems.map((subItem) => (
                      <li key={subItem.label}>
                        <a href="javascript:void(0);" onClick={subItem.onClick}>
                          <i className={subItem.icon + " fa-fw "}></i>
                          {subItem.label}
                        </a>
                      </li>
                    ))}
      ...
      expanded = (
        <div className="expanded-panel">
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            {task.processing_node_name || "-"} (
                            {task.auto_processing_node
                              ? _("auto")
                              : _("manual")}
                            )
                          </div>
                ...
                <div className="task-warning">
                  ...
                  <div className="inline">
                    <Trans
                      params={{
                        link: `<a href="${
                          window.__taskOptionsDocsLink
                        }" target="_blank">${window.__taskOptionsDocsLink.replace(
                          "https://",
                          ""
                        )}</a>`,
                      }}
                    >
                      {_(
                        '"Process exited with code 1" means that part of the processing failed. Sometimes it\'s a problem with the dataset, sometimes it can be solved by tweaking the Task Options. Check the documentation at %(link)s'
                      )}
                    </Trans>
                  </div>
                </div>
          ...
          <div className="row clearfix">{actionButtons}</div>
    ...
    if (
      editable ||
      (!task.processing_node && this.props.hasPermission("change"))
    ) {
      taskActions.push(
        <li key="edit">
    ...
    if (this.props.hasPermission("delete")) {
      ...
      if (task.status === statusCodes.COMPLETED && !task.compacted) {
        addTaskAction(
          _("Compact"),
          "fa fa-database",
          this.genActionApiCall("compact", {
            confirm: _(
              "Compacting will free disk space by permanently deleting the original images used for processing. It will no longer be possible to restart the task. Maps and models will remain in place. Continue?"
            ),
    ...
    return (
      <div className="task-list-item">
        ...
        <div className="row">
          <div
            className="col-xs-7 col-sm-6 col-md-9 col-lg-9"
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "flex-start",
